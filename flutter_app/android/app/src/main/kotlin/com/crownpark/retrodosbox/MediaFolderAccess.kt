 ...
 *
 * What SAF hands back is a content:// document tree, not a path, and dosbox-x
 * mounts a DIRECTORY: `mount c <dir>` needs a real POSIX tree it can walk, so
 * unlike a single disk image there is nothing that can be materialised on
 * demand. The tree is therefore an import SOURCE: enumerated here, with each
 * entry's relative directory preserved so a game's folder structure survives,
 * and [copyDocument] writes the bytes into the app's own games folder - which
 * dosbox-x can then mount with no permission of any kind.
 */
object MediaFolderAccess {
	 ...
	 * halfway cannot leave a half-written executable in a game folder that
